...
@section('content-class', 'chat-page')
...
@section('content')
        ...
        <div class="chat__wrapper">
            <div class="trading-header">
                <div class="trading-title">
                    ...
                    @if (auth()->id() === $order->buyer_id && in_array($order->status, ['paid', 'trading']))
                    ...
                    @foreach ($messages as $message)
                        ...
                        @if ($message->user_id === $me->id)
                            <div class="chat-group__me" data-message-id="{{ $message->id }}">
                                ...
                                {{-- 通常表示 --}}
                                <div class="sent-message__message message-view">{{ $message->message }}</div>

                                {{-- 編集モード（最初は非表示） --}}
                                <form class="message-edit-form" action="{{ route('message.update', $message->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                        ...
                        @else
                            <div class="chat-group__other">
                                ...
                                <div class="sent-message__message">{{ $message->message }}</div>
                                @if ($message->image)
                                    <img class="chat-image"
                                        src="{{ asset('storage/images/chat_image/' . $message->image) }}" alt="添付画像">
                                @endif
                            </div>
                        @endif
                    @endforeach
            ...
            <div class="input-message__area">
                ...
                <form class="input-form" action="{{ route('message.store', $order->id) }}" method="post"
                    ...
                    @csrf
                    <textarea class="input-message" name="new_message" placeholder="取引メッセージを記入してください">{{ $messageDraft }}</textarea>
    ...
    <div id="ratingModal" class="modal">
        <div class="modal-content">
            ...
            <h1 class="modal-heading">取引が完了しました。</h1>
        ...
        function saveDraftAndGo(url, orderId) {
            fetch("{{ route('message.draft') }}", {
                ...
                body: JSON.stringify({
                    ...
                    message: document.querySelector('textarea[name="new_message"]').value
                })
            }).then(() => {
